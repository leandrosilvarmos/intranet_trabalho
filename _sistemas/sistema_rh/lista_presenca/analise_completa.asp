...
<%
    Dim vNrLista, vNmSolicitante, vTituloEvento, vAvaliacao, vResultadoLista, vAnalistaLista, vMotivoLista, vAnaliseFinalizada
    ...
    vAnaliseFinalizada = Request("analise_finalizada")
%>
...
<%
	...
	If vResultadoLista = "Cancelado" Then
        objListaPresenca.fAtualizaAnalise vNrLista, vAnalistaLista, vResultadoLista, vMotivoLista, "Sim", "AV CANCELADA DEVIDO LISTA CANCELADA"
	Else
        objListaPresenca.fAtualizaAnalise vNrLista, vAnalistaLista, vResultadoLista, vMotivoLista, vAnaliseFinalizada, ""
	End If
...
<%
	...
	If vResultadoLista = "Aprovado" Then
		objEmail.pTitulo = "AVISO - Lista de Presença APROVADA"

		MyBody = "Sistemas Interno HESAP - Lista de Presença "	 & vbcrlf & vbcrlf  & vbcrlf	
		MyBody = MyBody & "Sua lista de presença foi APROVADA pelo RH." & vbcrlf & vbcrlf
		MyBody = MyBody & "Veja abaixo os detalhes da lista. " & vbcrlf 
		MyBody = MyBody & "Título da Lista: "&vTituloEvento&""	& vbcrlf & vbcrlf
		MyBody = MyBody & "Tipo de Avaliação: "&vAvaliacao&""	& vbcrlf & vbcrlf
		MyBody = MyBody & "Observação: Não será aceito lista de presença impressa frente e verso." & vbcrlf & vbcrlf
		MyBody = MyBody & "Clique no link abaixo para imprimir essa lista de presença "  & vbcrlf  
		MyBody = MyBody & fGetUrlPage(EnumURL_ParcialUrl)&"impressao_lista.asp?lista="&vNrLista&"" & vbcrlf & vbcrlf 	
	...
	If vResultadoLista = "Rejeitado" Then
		objEmail.pTitulo = "AVISO - Lista de Presença REJEITADA"

		MyBody = "Sistemas Interno HESAP - Lista de Presença "	 & vbcrlf & vbcrlf  & vbcrlf			
		MyBody = MyBody & "Sua lista de presença foi REJEITADA pelo RH." & vbcrlf & vbcrlf 
		MyBody = MyBody & "Veja abaixo os detalhes da lista. " & vbcrlf
		MyBody = MyBody & "Título da Lista: "&vTituloEvento&""	& vbcrlf & vbcrlf
		MyBody = MyBody & "Tipo de Avaliação: "&vAvaliacao&""	& vbcrlf & vbcrlf
		MyBody = MyBody & "Motivo da REPROVAÇÃO: "&vMotivoLista&""	& vbcrlf & vbcrlf
		MyBody = MyBody & "O RH está aguardando a alteração desta lista. Clique no link para corrigi-la " & vbcrlf
		MyBody = MyBody & fGetUrlPage(EnumURL_ParcialUrl)&"correcao_lista.asp?lista="&vNrLista&"" & vbcrlf & vbcrlf 
    ...
    <div id="tudo">
        ...
        <section>
            <div id="conteudo">
                ...
                <br />
                <% If vResultadoLista = "Rejeitado" Then %>
                <p align="center" style="font-size: 18px">A lista foi REJEITADA. O solicitante receberá um e-mail para a alteração dos dados.</p>
                <br />
                <% Else 
                        If vResultadoLista = "Aprovado" Then %>
                            <p align="center" style="font-size: 18px">A lista foi APROVADA. O solicitante receberá um e-mail para a impressão da lista.</p>
                            <br />
                            <p align="center" style="font-size: 14px">
                                <a href="../lista_presenca/impressao_lista.asp?lista=<%=vNrLista%>">Clique aqui para impressão da sua lista.
                                        <img src="/_images/icon_printer.gif" border="0" />
                                </a>
                            </p>
                <%      Else %>
                ...
                <br />
                <p align="center">
                    <a href="../lista_presenca/analise.asp">Clique aqui para voltar para as análises pendentes.<br />
