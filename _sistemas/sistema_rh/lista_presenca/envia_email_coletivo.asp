...
<%
    ...
    If vLog <> "1" Then
        ...
        Dim arrEmails
        arrEmails = objListaPresenca.fArrayEmailEficaciaNaoFinalizadas(vDataPrazo)
    ...
    If vLog <> "1" Then
        ...
        For i = 0 to UBound(arrEmails, 1)
                If arrEmails(i, 3) <> "" Then
                    ...
                    MyBody = MyBody & "<p><span style='color:#ff0000;'>ATENÇÃO: A AVALIAÇÃO DE EFICÁCIA DO TREINAMENTO SOLICITADO DEVERÁ SER REALIZADA NO PRAZO ESTIPULADO NA SOLICITAÇÃO DE DESENVOLVIMENTO.</span></p>" & vbcrlf
                    ...
                    For j = 0 to UBound(numLista)
                            MyBody = MyBody & "<tr>"
                                MyBody = MyBody & "<td>"
                                    'MyBody = MyBody & "<a href='"&fGetUrlPage(EnumURL_ParcialUrl)&"avaliacao_eficacia.asp?lista="&numLista(j)&"'>"
                                        MyBody = MyBody & numLista(j)
                                    'MyBody = MyBody & "</a>"
                                MyBody = MyBody & "</td>"
                                MyBody = MyBody & "<td>"
                                    MyBody = MyBody & "<a href='"&fGetUrlPage(EnumURL_ParcialUrl)&"avaliacao_eficacia.asp?lista="&numLista(j)&"'>"
                                        MyBody = MyBody & Replace(titulo(j),"¬",",")
                                    MyBody = MyBody & "</a>"
                                MyBody = MyBody & "</td>"
                    ...
                    'Atualiza 'ENVIO_NOTIFICACAO_RH_AV' com a data atual
                    objListaPresenca.fGravaEnvioNotificacaoRhAvII arrEmails(i, 1) ' - NR_LISTA
...
<script type="text/javascript">
    window.location = 'envia_email_coletivo.asp?log=1';
</script>
<%      Response.End
   End If 
%>
    ...
    <div id="tudo">
        ...
        <section>
            <div id="conteudo">
                ...
                <br />
                <br />
                <p align="center" style="font-size: 18px">O e-mail de notificação (cobrança da AVALIAÇÃO DE EFICÁCIA) foi enviado a todos os solicitantes que tiverem listas com data de prazo de avaliação = ou < que a informada.</p><br />
                <br />
